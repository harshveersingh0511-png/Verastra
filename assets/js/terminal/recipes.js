...
export function runRecipe(cls, query, ent, profile) {
  ...
  /* ── Market overlay handoff (Phase 3) ──────────────────────────────
     Sync recipes stay sync. The overlay paragraph is async-fetched by
     terminal.js between the recipe call and the memo render, using
     this context object. All overlay logic (Firestore reads, gating,
     freshness, benchmark-comparability) lives in market-overlay.js →
     overlay-composer.js — terminal.js only awaits a string.

     ctx.kind === 'role_city' → role-city composition for (cluster, role, city)
     ctx.kind === 'city'      → city-pulse composition for (city)
     ctx.kind === 'system'    → dashboard-pulse system summary
     ctx === null             → no overlay; terminal renders benchmark-only memo */
  memo._overlayContext = _deriveOverlayContext(cls, ent, profile);
  return memo;
}

function _deriveOverlayContext(cls, ent, profile) {
  const p = ent && profile
    ? resolveProfileWithEnt(profile, ent)
    : (profile || {});
  const cityName = (p.city || ent?.cities?.[0]?.key) || null;

  switch (cls) {
    case 'CAPITAL_VALUATION':
    case 'OFFER_EVALUATION':
    case 'COHORT_POSITION':
    case 'TRAJECTORY_PROJECTION':
      return { kind: 'role_city', benchCluster: p.cluster, benchRole: p.role, cityName };
    case 'CITY_MOVE': {
      const dest = ent?.cities?.length >= 2 ? ent.cities[1].key
                 : ent?.cities?.length === 1 ? ent.cities[0].key
                 : (p.city === 'Bangalore' ? 'Hyderabad' : 'Bangalore');
      return { kind: 'city', cityName: dest };
    }
    case 'CLUSTER_PIVOT': {
      const altRoles = (ent?.roles || []).filter(r => r.cluster !== p.cluster);
      const bCluster = altRoles[0]?.cluster
        || (ent?.clusters || []).find(c => c.cluster !== p.cluster)?.cluster
        || (p.cluster === 'finance' ? 'consulting' : 'finance');
      const bRole = altRoles[0]?.role || B.listRoles(bCluster)[0]?.value;
      return { kind: 'role_city', benchCluster: bCluster, benchRole: bRole, cityName };
    }
    case 'PROMOTION_SCENARIO':
      return { kind: 'role_city', benchCluster: p.cluster, benchRole: p.role, cityName };
    case 'PATH_COMPARISON': {
      const r = (ent?.roles || [])[0];
      const bCluster = r?.cluster || p.cluster;
      const bRole = r?.role || p.role;
      return { kind: 'role_city', benchCluster: bCluster, benchRole: bRole, cityName };
    }
    case 'BENCHMARK_LOOKUP':
      return { kind: 'system' };
    case 'SKILL_INVESTMENT':
    case 'VOLATILITY_READ':
    case 'FOUNDER_TRACK':
    default:
      // Benchmark-only by design — terminal will render no overlay section.
      return null;
  }
}
